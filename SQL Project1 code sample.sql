...
- lấy ra vị trí khoảng trống, thì tên sẽ là vị trí khoảng trống cộng thêm khoảng 10 ký tự.
@HT: tên của function
@Ten: tên cần tách
@L: Độ dài
@i: Vị trí bất kỳ
@j: Vị trí của khoảng trống
@kt: vị trí bất kỳ + 1

-- ứng dụng hiển thị danh sách sinh viên: MASV, HOTEN, MALOP, NGAYSINH (dd/mm/yyyy), Gioitinh(Nam, Nữ) được sắp xếp theo
thứ tự ưu tiên Malop, Tên sinh viên.
USE PROJECT1
GO
SELECT MASV, HOTEN, MALOP, CONVERT(nvarchar, NGAYSINH, 103) AS NGAYSINH, CASE GIOITINH
WHEN 'FALSE' THEN 'NAM' ELSE N'Nữ' END AS Gioitinh, YEAR(NGAYSINH) AS Namsinh
FROM SINHVIEN
ORDER BY MaLop, dbo.Tachten(Hoten) ASC

SELECT MASV, dbo.Tachten(Hoten) AS TEN
FROM SINHVIEN	